...
const Sidebar = ({ activeTab, setActiveTab, theme, toggleTheme, syncStatus, forceSync }) => {
  ...
  const menuItems = [
    ...
    { id: 'calendar', label: 'Jadwal Shift', icon: Calendar },
  ...
  const NavButton = ({ item, isActive }) => (
    <button
      ...
      style={{
        ...
        transform: hoveredItem === item.id && !isActive ? 'translateX(4px)' : 'translateX(0)',
      }}
    >
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem', flex: 1, overflow: 'auto' }}>
          ...
          <div style={{ margin: '0.75rem 0', padding: '0.65rem', borderRadius: 'var(--radius-md)', background: 'var(--bg-card)', border: '1px solid var(--glass-border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '1px' }}>
              {['M','S','S','R','K','J','S'].map((d,i) => (<div key={i} style={{ fontSize: '0.5rem', color: 'var(--text-muted)', textAlign: 'center', padding: '1px' }}>{d}</div>))}
